...
void handle_two_finger_gestures(const struct device *dev, const struct iqs5xx_rawdata *data, struct gesture_state *state) {
    ...
    // BULLETPROOF ONE-SHOT ZOOM - Only if both fingers have good contact
    if (data->fingers[0].strength > 500 && data->fingers[1].strength > 500) {

        // Early exit if zoom already triggered for this gesture
        if (zoom_already_triggered) {
            LOG_DBG("Zoom blocked: already triggered this gesture");
            return;
        }
        ...
        LOG_DBG("Zoom: initial=%.1f, current=%.1f, change=%.1f, triggered=%d",
                (double)initial_gesture_distance, (double)current_distance,
                (double)distance_change, zoom_already_triggered);

        // TRIGGER: Significant movement (100px threshold for reliability)
        if (fabsf(distance_change) > 100.0f) {

            if (distance_change > 0) {
                // Fingers moving apart = ZOOM IN
                LOG_INF("*** ZOOM IN: Pinch apart (%.1f px) ***", (double)distance_change);
                send_trackpad_zoom_in();
            } else {
                // Fingers moving together = ZOOM OUT
                LOG_INF("*** ZOOM OUT: Pinch together (%.1f px) ***", (double)distance_change);
