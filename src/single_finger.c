...
void handle_single_finger_gestures(const struct device *dev, const struct iqs5xx_rawdata *data, struct gesture_state *state) {
    // IMMEDIATE hardware gesture handling - NO DELAYS, NO DOUBLE-TAP DETECTION
    if (data->gestures0) {
    ...
    if (data->finger_count == 1) {
        ...
        if (data->rx != 0 || data->ry != 0) {
            // Correct axis mapping: rx=X movement, ry=Y movement (inverted)
            ...
            // Lower threshold for responsive movement
