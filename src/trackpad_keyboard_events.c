// src/trackpad_keyboard_events.c - Fixed ZMK API usage
...
static void f3_release_cb(struct k_work *work) {
    ...
    ZMK_EVENT_RAISE(zmk_keycode_state_changed, release_event);
}
...
static void zoom_in_modifier_release_cb(struct k_work *work) {
    ...
    ZMK_EVENT_RAISE(zmk_keycode_state_changed, key_release);
    ...
    ZMK_EVENT_RAISE(zmk_keycode_state_changed, mod_release);
}
